...
<code class="html" x-include="/admin-console/components/views/Ticket/get.html"></code>
The source of the above example is located at: /admin-console/components/views/Ticket/get.html 

#### Basic concept
```html
<draftsman-query alias="Ticketget" x-include="/prepared-statements/views/Ticket/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/Ticket/get.txt) to an Alpine data object (Ticketget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'Ticket'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.Ticketget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="Ticketget" x-include="/form-definitions/Ticket/get.json"></div>
```
...
from the view updater indicating that view 'Ticket' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.ticket.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.ticket.get.key' refers to previously retrieved data using a query tag.
```html
...
<code class="html" x-include="/admin-console/components/commands/VerifyTicket.html"></code>

#### The basic concept
```html
<draftsman-mutation command="VerifyTicket" x-include="/prepared-statements/commands/VerifyTicket.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/VerifyTicket.txt) as a command (VerifyTicket) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="VerifyTicket" x-include="/form-definitions/VerifyTicket.json"></div>
```
...
```javascript
$store.mutation.send('VerifyTicket',{all-form-data});
```
...
```html
<draftsman-trace command="VerifyTicket" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'VerifyTicket', data: {} });">
</draftsman-trace>
```
