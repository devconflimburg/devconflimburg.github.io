...
<code class="html" x-include="/admin-console/components/commands/UpdateWebSiteElement.html"></code>

#### The basic concept
```html
<draftsman-mutation command="UpdateWebSiteElement" x-include="/prepared-statements/commands/UpdateWebSiteElement.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/UpdateWebSiteElement.txt) as a command (UpdateWebSiteElement) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="UpdateWebSiteElement" x-include="/form-definitions/UpdateWebSiteElement.json"></div>
```
...
```javascript
$store.mutation.send('UpdateWebSiteElement',{all-form-data});
```
...
```html
<draftsman-trace command="UpdateWebSiteElement" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'UpdateWebSiteElement', data: {} });">
</draftsman-trace>
```
