...
<code class="html" x-include="/admin-console/components/commands/ImportSubscriptionEmail.html"></code>

#### The basic concept
```html
<draftsman-mutation command="ImportSubscriptionEmail" x-include="/prepared-statements/commands/ImportSubscriptionEmail.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/ImportSubscriptionEmail.txt) as a command (ImportSubscriptionEmail) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="ImportSubscriptionEmail" x-include="/form-definitions/ImportSubscriptionEmail.json"></div>
```
...
```javascript
$store.mutation.send('ImportSubscriptionEmail',{all-form-data});
```
...
```html
<draftsman-trace command="ImportSubscriptionEmail" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'ImportSubscriptionEmail', data: {} });">
</draftsman-trace>
```
