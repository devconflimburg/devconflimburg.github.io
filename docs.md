...
<code class="html" x-include="/admin-console/components/commands/RedeemTicket.html"></code>

#### The basic concept
```html
<draftsman-mutation command="RedeemTicket" x-include="/prepared-statements/commands/RedeemTicket.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/RedeemTicket.txt) as a command (RedeemTicket) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="RedeemTicket" x-include="/form-definitions/RedeemTicket.json"></div>
```
...
```javascript
$store.mutation.send('RedeemTicket',{all-form-data});
```
...
```html
<draftsman-trace command="RedeemTicket" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'RedeemTicket', data: {} });">
</draftsman-trace>
```
