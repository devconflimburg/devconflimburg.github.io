...
<code class="html" x-include="/admin-console/components/views/RegistrationManager/get.html"></code>
The source of the above example is located at: /admin-console/components/views/RegistrationManager/get.html 

#### Basic concept
```html
<draftsman-query alias="RegistrationManagerget" x-include="/prepared-statements/views/RegistrationManager/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/RegistrationManager/get.txt) to an Alpine data object (RegistrationManagerget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'RegistrationManager'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.RegistrationManagerget.RegistrationManager.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="RegistrationManagerget" x-include="/form-definitions/RegistrationManager/get.json"></div>
```
...
from the view updater indicating that view 'RegistrationManager' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.registrationmanager.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.registrationmanager.get.key' refers to previously retrieved data using a query tag.
```html
...
<code class="html" x-include="/admin-console/components/commands/ScheduleEmail.html"></code>

#### The basic concept
```html
<draftsman-mutation command="ScheduleEmail" x-include="/prepared-statements/commands/ScheduleEmail.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/ScheduleEmail.txt) as a command (ScheduleEmail) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="ScheduleEmail" x-include="/form-definitions/ScheduleEmail.json"></div>
```
...
```javascript
$store.mutation.send('ScheduleEmail',{all-form-data});
```
...
```html
<draftsman-trace command="ScheduleEmail" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'ScheduleEmail', data: {} });">
</draftsman-trace>
```
